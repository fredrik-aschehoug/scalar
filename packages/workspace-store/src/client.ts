...
export const createWorkspaceStore = (workspaceProps?: WorkspaceProps): WorkspaceStore => {
  ...
  async function addInMemoryDocument(
    ...
    // build a small navigation tree (Messages section only) so the sidebar
    // can surface `components.messages`. Reference resolution inside payloads
    // remains out of scope for the MVP — the renderer dereferences manually.
